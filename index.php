...
<link rel="stylesheet" href="<?php echo $url; ?>css/dot-luv/jquery-ui-1.10.3.custom.min" type="text/css" />
<link rel="stylesheet" href="<?php echo $url; ?>css/global.css" type="text/css" />
<script src="<?php echo $url; ?>js/jquery-1.9.1.js" type="text/javascript"></script>
<script src="<?php echo $url; ?>js/jquery-ui-1.10.3.custom.min.js" type="text/javascript"></script>
<?php
...
if(isset($_SESSION['user']['id']) && isset($_SESSION['user']['location'])) {
	...
	if($_SESSION['user']['location'] != $_SERVER['REMOTE_ADDR']) {
		...
		if($user) {
			$user['access'] = $userModel->GetAccessRights($_SESSION['user']['id']);
		}
	}	// end if-else
} else {
